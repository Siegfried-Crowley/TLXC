...
function displayResult(submission) {
    ...
    var statusClass = submission.status === 'accepted' ? 'status-accepted' :
        submission.status === 'wrong_answer' ? 'status-wrong_answer' : 'status-error';
    var html = '<div class="result-item"><span class="result-label">状态</span><span class="submission-status ' + statusClass + '">' + getStatusText(submission.status) + '</span></div>' +
        '<div class="result-item"><span class="result-label">运行时间:</span><span class="result-value">' + (submission.runtimeMs || 0) + ' ms</span></div>' +
        '<div class="result-item"><span class="result-label">测试用例:</span><span class="result-value">' + (submission.passedCases || 0) + ' / ' + (submission.totalCases || 0) + '</span></div>';
    ...
    html += '<div class="result-item"><span class="result-label">积分变化:</span><span class="result-value">+' + (submission.scoreDelta || 0) + '</span></div>' +
